#include "utility.h"
void plotProposalFigure_1_ratio(TString name="phi", int smear_ = 0, int PHP_ = 0, int veto_ = 1, double minPt_=0.15, int method=2){
	...
	if(name=="phi"){base11->GetYaxis()->SetRangeUser(4e-2, 1e7);}
	...
	*/
	...
	TLegend *w6 = new TLegend(0.53,0.62,0.9,0.77);
	w6->SetLineColor(kWhite);
	w6->SetFillColor(0);
	w6->SetTextSize(15);
	w6->SetTextFont(45);
	w6->AddEntry(h_coh_sartre, "Sartre coherent #phi truth ", "PL");
	w6->AddEntry(h_total, "Total-incoh. #phi reco'. "+method_name+"", "PE");
	w6->AddEntry(h_total_subtract, "Total-incoh. residue-subtracted #phi" , "PE");
	w6->AddEntry(h_total_subtract, " reco'. "+method_name+"", "");
	w6->Draw("same");

	TBox* box1 = new TBox(0.088,5.6E4,0.098,1.2E5);
    box1->SetFillColorAlpha(kRed,0.3);
    box1->SetFillStyle(1001);
	box1->SetLineWidth(0);
	// box1->Draw("same");

	TBox* box2 = new TBox(0.088,5.5E4,0.098,1.2E5);
	...
	TH1D* base111 = makeHist("base111", "", "|#it{t} | (GeV^{2})", "d#sigma / d|#it{t} |  (nb/GeV^{2}) ", 100,0,0.18,kBlack);
	base111->GetYaxis()->SetRangeUser(-30, 45);
	base111->GetXaxis()->SetTitleColor(kBlack);
	TGaxis::SetMaxDigits(3);
	fixedFontHist1D(base111,6,5);
	base111->GetYaxis()->SetTitleSize(base111->GetYaxis()->GetTitleSize()*0.6);
	base111->GetXaxis()->SetTitleSize(base111->GetXaxis()->GetTitleSize()*0.6);
	base111->GetYaxis()->SetLabelSize(base111->GetYaxis()->GetLabelSize()*0.6);
	base111->GetXaxis()->SetLabelSize(base111->GetXaxis()->GetLabelSize()*0.6);
	base111->GetXaxis()->SetNdivisions(4,4,0);
	base111->GetYaxis()->SetNdivisions(5,5,0);
	base111->Draw();
	h_coh_sartre->Draw("hist same");
	TH1D* h_total_copy = (TH1D*) h_total->Clone("h_total_copy");
	TGraphErrors* gr_copy = (TGraphErrors*) gr->Clone("gr_copy");
	h_total_copy->SetMarkerSize(0.4);
	gr_copy->SetMarkerSize(0.4);
	h_total_copy->Draw("P E1 same");
	gr_copy->Draw("P C e3 same");
	...
	TH1D* base22 = makeHist("base22", "", "|#it{t} | (GeV^{2})", "reco' / truth ", 100,0,0.18,kBlack);
	base22->GetYaxis()->SetRangeUser(0, 2.2);
	...
	// TFile* output_file = new TFile("proposal_0.75T.root","RECREATE");
	// TGraphErrors* gr_0p75T = (TGraphErrors*) gr->Clone("gr_0p75T");
	// TH1D* h_0p75T = (TH1D*) h_total_subtract->Clone("h_0p75T");
	// gr_0p75T->Write();
	// h_0p75T->Write();

	c1->Print(Form("../figures/proposal_ideal/proposal_figure_PHP_%d_veto_%d_minPt_%.2f_smear_%d.pdf",PHP_,veto_,minPt_,smear_));
