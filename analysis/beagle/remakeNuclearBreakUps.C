...
void remakeNuclearBreakUps(const TString filename="eA_TEST", const int nEvents = 40000, bool PHP_ = false){
	...
	for(int iveto=0;iveto<2;iveto++){
		for(int iprocess=0;iprocess<2;iprocess++){
			for(int ivm=0;ivm<3;ivm++){
				h_VM_t[iveto][iprocess][ivm] = new TH1D(Form("h_VM_t_%d_%d_%d",iveto,iprocess,ivm),";-t (GeV^{2})",400,0.,2);
			}
		}
	}
	TH1D* h_photon[3];
	...
	for(int ivm=0;ivm<3;ivm++){
		...
		h_w_photon[ivm] = new TH2D(Form("h_w_photon%d",ivm),Form("h_w_photon%d",ivm),100,0,0.2,100,1.5,90);
	}
	for(int i(0); i < nEvents; ++i ) {
		...
		for(int ivm=0;ivm<3;ivm++){
			if(hasvm[ivm]){
				...
				h_w_photon[ivm]->Fill(photon_flux, sqrt(trueW2));
